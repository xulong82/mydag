function [sampled_graphs, accept_ratio, num_edges] = learn_struct_mcmc_from_prior_MBmoves3(output,data, ns,initial_dag,varargin)
...
for t=1:burnin
 [dag, nbrs, ops, nodes, A, accept] = take_step(output,dag, nbrs, ops, ...
...
function [new_dag, new_nbrs, new_ops, new_nodes, A,  accept] = ...
   take_step(output,dag, nbrs, ops, nodes, ns, data, clamped, A,  ...
     scoring_fn, discrete, type, params, lML)
...
x = rand;  
    ...
    [new_dag,R,n_child] = MBR_move(ns,output,dag);
    R1=R
    
    [nbrs, ops, nodes, A] = mk_nbrs_of_digraph(dag);
    [new_nbrs, new_ops, new_nodes,new_A] = mk_nbrs_of_digraph(new_dag); 
        end
   else
